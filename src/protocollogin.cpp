...
bool ProtocolLogin::parseFirstPacket(NetworkMessage& msg)
{
	...
	if(!RSA_decrypt(msg))
	{
		disconnectClient(0x0A, "Utilize o Client oficial.");
		return false;
	}
	...
	if(name.empty())
	{
		...
		{
			disconnectClient(0x0A, "Account name vazia, crie sua conta no site.");
			return false;
		}
	...
	{
		disconnectClient(0x0A, "Servidor está em manuntenção.");
		return false;
	}
	...
	if(!IOLoginData::getInstance()->getAccountId(name, id))
	{
		...
		disconnectClient(0x0A, "Account name ou Password incorreto.");
		return false;
	}
