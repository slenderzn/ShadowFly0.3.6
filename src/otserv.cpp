...
bool argumentsHandler(StringVec args)
{
	...
	for(StringVec::iterator it = args.begin(); it != args.end(); ++it)
	{
		...
		if((*it) == "--version")
		{
			...
			"A server developed by: Lucas Silva.\n"
			"otPokemon Diamond - Pokemon Tibia Online.\n";
			return false;
		}
...
ServiceManager* services)
{
	...
	std::cout << "A server developed by : Lucas  Silva." << std::endl;
	std::cout << "OtPokemon Diamond - Pokemon Tibia Online." << std::endl << std::endl;
    ...
    **/
	...
	const char* p("13257169195116060917166547523384277415613733237685550001331180889375348352108371643562410819417519379273994109047614913784931534611588942200319312571604321");
	const char* q("7823533889782528752451700150770179446847474728357575804260026541080984125994085909563678062745132617699648764237318762224552286332577474761205904722213029");
	const char* d("");
	g_RSA.setKey(p, q, d);
			...
			switch(LOWORD(wParam))
			{
				...
				case ID_MENU_MAIN_CLEARLOG:
				{
					if(g_game.getGameState() != GAME_STATE_STARTUP)
					{
						...
						std::cout << "Compiled with " << BOOST_COMPILER << " at " << __DATE__ << ", " << __TIME__ << "." << std::endl;
						std::cout << "A server developed by: Lucas Silva." << std::endl;
						std::cout << "OtPokemon Diamond - Pokemon Tibia Online ." << std::endl;
						std::cout << std::endl;
					}
